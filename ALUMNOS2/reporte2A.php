...
<?php
       ...
       $sql = "SELECT  *  FROM   reportes, tb_residentes
           ...
           $query = $db->execute($sql);
            while($datos=$db->fetch_row($query)){?>
            <tr>
            <td><?php echo $datos['idreporte']; ?></td>
            <td><?php echo $datos['no_control']; ?></td>
            <td><?php echo $datos['nombre']." ".$datos['ap']." ".$datos['am']; ?></td>
            <td><?php echo $datos['carrera']; ?></td>
            
              <td><a href="../ASESORES2/archivosreportedosA.php?id=<?php echo strtoupper($datos['no_control'])?>" ><?php echo $datos['nombre_archivoR2']; ?></a></td>   
