...
/*
  ...
  $pdf->SetFont('Arial', '', 10);
  $pdf->SetXY(20, 148);
  $pdf->MultiCell(180, 4,  utf8_decode("Las situaciones no previstas en el presente acuerdo serán analizadas por el Departamento de Residencias Profesionales y Servicio Social.
"), 0, 'J', 0);
...
$pdf->SetXY(25, 180);
$pdf->Cell(0, 5, utf8_decode("C. Jesús Octavio Contreras Cruz"), 0, 1, 'la');
...
$pdf->Image('sello.jpg', 80, 193, 33);
$pdf->Image('firma.png', 40, 204, 28);
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetXY(18, 220);
$pdf->Cell(0, 5, utf8_decode("C. María Del Carmen Martinéz Escobar "), 0, 1, 'la');
...
while ($rowe = mysqli_fetch_array($rec2)) {
  ...
  if ($array_cadena >= 0 and $array_cadena <=90) {
    $pdf->SetXY(65, 180);
    $pdf->MultiCell(180, 3,  utf8_decode($rowe['GradoAT'].' '.$rowe['NombreAC']), 0, 'C', 0);
    $pdf->SetFont('Arial', '', 10);
    $pdf->SetXY(65, 185.5);
    $pdf->MultiCell(180, 2,  utf8_decode($rowe['PuestoAC']), 0, 'C', 0);
    $pdf->SetXY(115,  190);
  ...
  if ($array_cadena >=91 and $array_cadena <=120) {
    ...
    $pdf->SetFont('Arial', '', 10);
    $pdf->SetXY(110, 184);
    $pdf->MultiCell(95, 3,  utf8_decode($rowe['CargoPrograma']), 0, 'C', 0);
    $pdf->SetXY(115,  191);
    $pdf->MultiCell(80, 4.5,  utf8_decode($rowe['Dependencia']), 0, 'C', 0);
  }
}
